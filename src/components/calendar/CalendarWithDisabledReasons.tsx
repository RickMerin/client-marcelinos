...
const BOOKING_CONFLICT_REASON = "Fully booked for this date. ";
...
function DayWithReason({
  ...
  const isBookingConflict =
    isBlocked && (reason === "Booked" || reason === "Fully booked");
  ...
  const selectedInStayRange =
    isSelectedDay && inStayRangeHighlight && !isBlocked;
...
export function CalendarWithDisabledReasons({
  ...
  const [activeTooltipDate, setActiveTooltipDate] = React.useState<
    string | null
  >(null);
