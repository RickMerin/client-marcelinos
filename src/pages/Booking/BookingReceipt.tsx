...
export function BookingReceipt() {
  ...
  if (!reference_id) return <p className="text-center mt-10">No reference provided</p>;
  if (isLoading) return <p className="text-center mt-10">Loading receipt...</p>;
  if (isError || !data) return <p className="text-center mt-10">Receipt not found</p>;
    ...
    <div className="flex justify-center p-4 mt-10">
      <div className="w-full max-w-2xl bg-white border border-gray-200 rounded-lg shadow-md p-6 font-sans">
        ...
        <div className="text-sm mb-4">
          <div className="flex justify-between">
            <span>Subtotal:</span>
            <span>
              ₱{data.subtotal.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
            </span>
          </div>
          <div className="flex justify-between font-bold text-gray-900 text-base mt-2">
            <span>Grand Total:</span>
            <span>
              ₱{data.grand_total.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
            </span>
          </div>
        </div>
      ...
      <div className="flex flex-col items-center gap-0 leading-tight">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-xs text-gray-500">Payment Method: {data.payment_method}</p>
