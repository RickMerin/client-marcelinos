...
export function MultiStepForm() {
  ...
  const [submitConflictNotice, setSubmitConflictNotice] = useState("");
  ...
  useEffect(() => {
    ...
    const checkOutChanged =
      formData.check_out !== dateConflictSnapshot.checkOut;

    if (checkInChanged || checkOutChanged) {
      setDateConflictNotice("");
      setDateConflictSnapshot(null);
    }
  }, [dateConflictSnapshot, formData.check_in, formData.check_out]);

  useEffect(() => {
    if (!submitConflictNotice) return;
    setSubmitConflictNotice("");
  }, [selectedRoomIdsSignature, formData.check_in, formData.check_out]);
  ...
  const handleSubmit = async (websiteHoneypot: string) => {
    ...
    await submitBooking(
      ...
      (error) => {
        ...
        setSubmitConflictNotice(message);
          ...
          {formData.current_step === 4 && (
            <motion.div key="step4" {...stepMotion}>
              <Step4
                ...
                submitConflictNotice={submitConflictNotice}
                onBack={() => goToStep(3)}
