...
- Preferred API payload field:
  - `billing_statement_pdf_url`
  - This should be generated by the backend with Laravel's `URL::temporarySignedRoute(...)`
- Fallback API call when a signed URL is not provided:
...
Recommended signed-URL flow:

1. Generate a temporary signed route for the PDF endpoint with Laravel's built-in URL helper.
2. Include the signed URL in the booking receipt payload as `billing_statement_pdf_url`.
3. Let the frontend prefer that signed URL and fall back to the legacy reference-based endpoint only when the field is missing.
